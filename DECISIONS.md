...
## 42. Day-by-day itinerary proposal: collaborative route structuring with 3-way human review

**Call:** Added `propose_day_block` tool (17th tool in the WebMCP surface). When called, it stores a pending proposal (kind `"day-block"`) in the `Proposal` table containing the day's title (`label`), trail notes / mileage / elevation (`text`), and agent reasoning (`note`). The human review banner (`DayBlockBanner`) presents three distinct actions: `Accept & Add` (places the card populated with trail milestones), `Add as Blank Day` (places the card with empty text for human authoring), and `Dismiss` (drops the proposal). Direct day block authoring by an agent (`POST /api/board/place` with `itemType: "day"`, `addedBy: "agent"`) remains strictly blocked (`403 Forbidden`).

**Why:** When planning trips to unfamiliar destinations, users often lack knowledge of trail distances, elevation gain, and campsite milestones. Proposing day blocks one by one with explicit human review allows the agent to provide grounded itinerary recommendations without taking over the board or violating the core trust boundary: agents propose structure and arrange gear, humans decide and author.
